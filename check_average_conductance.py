...
def parse_args():
    parser = argparse.ArgumentParser(description="Calculate average conductance curves across parameter space.")
    parser.add_argument("--dirname", type=str, default="Data/Tdis_pfaff4", help="Path to data directory")
    parser.add_argument("--outdir", type=str, default=None, help="Directory to save plots (default: dirname/Plots/Average_Conductance)")
...
def make_plot(x: np.ndarray, mean_cond: np.ndarray, std_cond: np.ndarray, sem_cond: np.ndarray,
              xlabel: str, title: str, outfile: Path, color="forestgreen"):
    ...
    # Plot mean curve
    ax.plot(x, mean_cond, color=color, linewidth=2.5, label="Average Conductance")
    
    # Plot error bars
    step = max(1, len(x) // 25)
    ax.errorbar(x[::step], mean_cond[::step], yerr=std_cond[::step],
                fmt='o', color=color, markersize=4, capsize=3, elinewidth=1.2,
                linestyle='None', label=r"Error Bars ($\pm 1\sigma$)")
    
    ax.set_xlabel(xlabel, fontsize=13)
    ax.set_ylabel(r"Conductance $G / G_{\mathrm{norm}}$", fontsize=13)
...
def main():
    ...
    outdir = Path(args.outdir) if args.outdir else dirname / "Plots" / "Average_Conductance"
    ...
    for key, (arr, xlabel, label_str) in configs.items():
        ...
        mean_cond = np.mean(normed, axis=0)
        std_cond = np.std(normed, axis=0)
        sem_cond = std_cond / np.sqrt(M)
        
        stats_save[f"{key}_max_mean"] = mean_cond
        stats_save[f"{key}_max_std"] = std_cond
        stats_save[f"{key}_max_sem"] = sem_cond

        outfile = outdir / f"{key}_cond_max.png"
        title = f"Average Conductance (Max Normalized) - {label_str}"
        make_plot(x_data, mean_cond, std_cond, sem_cond, xlabel, title, outfile, color="forestgreen")
    ...
    for key, (arr, xlabel, label_str) in configs.items():
        ...
        mean_cond = np.mean(normed[valid_mask], axis=0)
        std_cond = np.std(normed[valid_mask], axis=0)
        sem_cond = std_cond / np.sqrt(np.sum(valid_mask))
        
        stats_save[f"{key}_sym_mean"] = mean_cond
        stats_save[f"{key}_sym_std"] = std_cond
        stats_save[f"{key}_sym_sem"] = sem_cond

        outfile = outdir / f"{key}_cond_sym.png"
        title = f"Average Conductance (Symmetric Barrier Normalized) - {label_str}"
        make_plot(x_data, mean_cond, std_cond, sem_cond, xlabel, title, outfile, color="darkviolet")

    # Save summary stats
    stats_file = outdir / "conductance_stats.npz"
    np.savez(stats_file, **stats_save)
    print(f"\nSaved aggregated conductance statistics to: {stats_file}")

    # Also generate summary 4-panel figures for quick comparison
    for norm_type, color, norm_name in [("max", "forestgreen", "Max Value"), ("sym", "darkviolet", "Symmetric Barrier")]:
        fig, axes = plt.subplots(2, 2, figsize=(14, 10), dpi=150)
        axes = axes.flatten()
        for i, (key, (_, xlabel, label_str)) in enumerate(configs.items()):
            ax = axes[i]
            mean_c = stats_save[f"{key}_{norm_type}_mean"]
            std_c = stats_save[f"{key}_{norm_type}_std"]
            
            ax.fill_between(x_data, mean_c - std_c, mean_c + std_c, color=color, alpha=0.2, label=r"$\pm 1\sigma$")
            ax.plot(x_data, mean_c, color=color, linewidth=2.0, label="Average Conductance")
            step = max(1, len(x_data) // 20)
            ax.errorbar(x_data[::step], mean_c[::step], yerr=std_c[::step], fmt='o', color=color,
                        markersize=3.5, capsize=2.5, elinewidth=1.0, linestyle='None')
            ax.set_xlabel(xlabel, fontsize=12)
            ax.set_ylabel(r"$G / G_{\mathrm{norm}}$", fontsize=12)
            ax.set_title(label_str, fontsize=13)
            ax.grid(True, linestyle=':', alpha=0.6)
            if i == 0:
                ax.legend(fontsize=10)
        
        fig.suptitle(f"Average Conductance Across All {M} Curves ({norm_name} Normalized)", fontsize=15, y=0.98)
        fig.tight_layout()
        fig.subplots_adjust(top=0.92)
        summary_out = outdir / f"all_conds_{norm_type}_summary.png"
